...
wrisky 的 Sparkle fork 打包。包名故意和 AUR 上的 `sparkle-git` / `sparkle-bin` /
...
makepkg -f
sudo pacman -U sparkle-fork-git-*.pkg.tar.zst
```
...
首装会顶掉 `sparkle` / `sparkle-git` / `sparkle-rolling-bin` 等（PKGBUILD 里写了 conflicts），
pacman 会问一次是否替换。
...
- 想回上游版本：`sudo pacman -Rns sparkle-fork-git && paru -S sparkle-rolling-bin`。
